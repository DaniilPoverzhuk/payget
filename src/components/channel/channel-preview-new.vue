      ...
      <div class="channel__head head">
        <div class="head__info info">
          <router-link
            ...
            :to="{ path: `/user/${post.user.name}/channel` }"
          >
            <img
              class="info__img"
              v-if="
          ...
          <div class="flex-shrink-0" :class="isRouterLink && 'link'" v-else>
            <img
              class="info__img"
              v-if="
                post.user && post.user.media !== null && post.user.media.avatar
          ...
          <div class="info__wrap">
            <div class="info__title">
              <template v-if="post.user">
                ...
                >
                <p class="info__text" :class="isRouterLink && 'link'" v-else>
                  {{ post.user.name }}
                </p>
              </template>
      ...
      <template v-if="post.tags && post.tags.length">
        <div class="taglist">
          <div
            ...
            "
          >
            <router-link :to="`/feeds/tag/${tag.slug}?tab=${activeTab}`">
...
export default {
  ...
  computed: {
    ...
    accessPost() {
      if (
        Array.isArray(this.post.private) ||
      ...
      const oneTimeSubscription = !Array.isArray(
        this.post.private.one_time_subscription
      )
      const plan_subscription =
        !Array.isArray(this.post.private.plan_subscription) &&
        new Date() < new Date(this.post.private.plan_subscription.end_date) &&
        new Date() > new Date(this.post.private.plan_subscription.start_date)
  ...
  async created() {
    this.info = this.channelInfo
    if (this.post && this.post.reactions !== null) {
      await this.$nextTick()
  ...
  methods: {
    ...
    onClickTag(tag) {
      this.$emit(
        'takeTag',
        this.tagActive && this.tagActiveId == tag.id ? null : tag
      )
    },
    async complaintType() {
      try {
