<template>
  <div class="commentPost__inner">
    <template v-if="!success">
      <div class="commentPost__wrap">
        <h2 class="commentPost__title">{{ $t('complaints.complain') }}</h2>
        <div class="commentPost__item">
          <q-select
            v-model="active"
            :options="types"
            option-value="id"
        ...
        <div class="commentPost__actions">
          <button
            class="button button_type_info commentPost__btn"
            @click="$emit('hide')"
          >
            {{ $t('button.cancel') }}
          </button>
          <button
            class="button commentPost__btn"
            :disabled="isLoading"
            @click="commentPost"
          >
            {{ $t('button.send') }}
...
export default {
  ...
  methods: {
    ...
    async submitPost() {
      ...
      form.append('channel_post_comment_id', this.postCommentId)
      ...
      try {
        const { status } = await this.$instance({
          url: `userfeed/post/${this.postCommentId}/complaint`,
          data: form,
        })
        if (status) {
          this.success = true
