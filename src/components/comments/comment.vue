<template>
  <div class="comment-wrap">
    <div v-if="isView && !isDetails" class="title-post">
      <router-link :to="`/feeds/${comment.post.slug}`">{{
        comment.post.title
      }}</router-link>
    </div>
    <div class="comment" :class="{ 'is-parent': !comment.reply_to }">
      <div class="comment__avatar">
        <!-- <img :src="comment.user.media.avatar" /> -->
        <CommentUserAvatar :src="comment.user.media.avatar" />
      </div>
      <div class="comment__body">
        ...
        <div class="comment__body-content" v-html="comment.comment"></div>
        ...
        >
          <template #comment v-if="isDetails && isMove">
            <router-link
              :to="`/user/${comment.user.name}/comments/${comment.id}`"
              >Перейти</router-link
            >
          </template>
        </CommentActions>
        ...
        <div
          class="comment__childrens"
          v-if="!isComplaint && childrenComments && childrenComments.length"
        >
          <Comment
            v-for="childrenComment in childrenComments"
            :key="childrenComment.id"
            :comment="childrenComment"
            :is-parent="false"
            @updateCount="emit('updateCount')"
          ></Comment>
          <ShowMore
            v-if="isShowMore"
            :loading="loading"
            class="show-more"
            @click="getCommentThread"
          />
        </div>
      </div>
    </div>
...
const emit = defineEmits(['updateCount', 'onComment'])
...
const isMove = computed(
  () =>
    $route.value?.params.subpage === 'comments' &&
    +$route.value?.params.slug !== props.comment.id
)
...
const getFormattedDate = computed(() =>
  formatDistance(new Date(props.comment.created_at + 'Z'), new Date(), {
    addSuffix: true,
...
const createComment = async (msg) => {
  const replyUser = isReplyUser.value
    ...
    : ''
  const firstPart = msg.slice(0, 3)
  const secondPart = msg.slice(3)
  msg = firstPart.concat(replyUser, secondPart)
  try {
    const { data, status } = await instance({
    ...
    isActiveSender.value = false
  }
}

const editComment = async (msg) => {
  try {
    const { data, status } = await instance({
    ...
    if (data.status == 'success') {
      props.comment.comment = msg
    }
    console.log('data, status', data, status)
  } catch (e) {
    console.log('Error edit comment /userfeed/comment', e)
    if (e.response.data.status == 'failure') {
      error.show(e.response.data.messages.comment[0])
    } else {
      error.show(e.response.data || 'Error')
