<template>
	<div class="disputes-aside h-full ">
		<div class="h-full flex flex-col" :class="isDisputesFooterVisible && 'disputes__body_new'">
			<div class="lg:flex lg:max-w-384 lg:items-center lg:justify-between mb-6 lg:pr-8">
				...
				<Search class="lg:ml-6" :query="searchQuery" :placeholder="$t('button.search')" @search="(query) => (searchQuery = query)" />
			</div>
			<TabsBadges :first-tab="disputesFilter" :labels="statusText" class="lg:pr-8" :stretch="true" :tabs-data="tabsData" @setPath="setPath" />
			<div class="lg:relative lg:flex lg:flex-grow lg:-ml-8">
				<div class="lg:absolute lg:inset-0 xs:mb-20">
					<div
						class="lg:h-full font-base lg:scrollbar-thin lg:scrollbar lg:scrollbar-thumb-gray-200 lg:dark:scrollbar-thumb-gray-700 lg:scrollbar-track-transparent lg:overflow-y-scroll lg:scrollbar-thumb-rounded-full lg:scrollbar-track-rounded-full">
						<ul v-if="localDisputes.length" class="block space-y-5 w-full ">
							<li v-for="dispute in localDisputes" :key="dispute.id" class="block lg:px-8">
								<DisputeCard class="lg:max-w-384" :dispute="dispute" />
							</li>
						</ul>
						<Loader v-if="quantity > visibleQuantity" v-observe-visibility="getDisputes" />
						<p v-if="localDisputes.length === 0" class="text-gray-500 lg:px-8 dark:text-gray-300 lg:mt-5 max-w-3xl xs:mt-3">
							{{ $t(noDisputesText) }}
						</p>
						<div v-if="isDisputesFooterVisible" class="lg:px-8 xs:fixed xs:inset-x-4 xs:bottom-0">
							<Button :loading="assigningDisputes" view-color="primary" class="w-full font-semibold py-3 mt-8 mb-4" @click="assignDisputes">
								{{ $t("disputes.takeDisputes", newQuantity) }}
							</Button>
						</div>
					</div>
				</div>
...
import DisputeCard from '../dispute-card/dispute-card.vue'
