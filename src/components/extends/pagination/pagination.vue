<template>
  <div
    v-if="count > 0 && pages.length > 1"
    class="pagination border-t border-gray-200 px-4 flex items-center justify-between"
  >
    <div class="-mt-px w-0 flex-1 flex">
      <button
        v-if="currentPage !== 1"
        class="border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm font-medium text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300"
        @click="prevPage"
      >
        <svg
          v-svg
          symbol="back-arrow"
          class="h-5 w-5 text-gray-400 dark:text-gray-500 mr-1"
        ></svg>
        {{ $t('previous') }}
      </button>
    </div>
    <div class="xs:hidden flex">
      <button
        v-for="page in pages"
        :key="page"
        class="cursor-pointer border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium"
        :class="{
          'text-primary border-primary': activePage(page),
          'text-primary border-transparent': !activePage(page),
        }"
    ...
    <div class="-mt-px w-0 flex-1 flex justify-end">
      <button
        ...
        class="border-t-2 border-transparent pt-4 pl-1 inline-flex items-center text-sm font-medium text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-primary hover:border-gray-300"
        @click="nextPage"
      >
        {{ $t('next') }}
        <svg
          v-svg
          symbol="back-arrow"
          class="h-5 w-5 text-gray-400 dark:text-gray-500 ml-1 transform rotate-180"
        ></svg>
      </button>
    </div>
  </div>
