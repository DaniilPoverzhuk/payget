  ...
  <div>
    ...
    >
      ...
      >
        <div class="flex items-center">
          <CurrencyIcon v-if="selectedOption" :name="selectedOption" />
          <span
            class="block truncate text-sm"
            :class="{ 'ml-3': !!selectedOption }"
          >
            {{ $t(setText) }}
          </span>
        </div>
      ...
      >
        <li
          ...
          :id="list.value"
          :key="list.value"
          class="text-gray-900 flex items-center dark:text-white cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-primary hover:text-white"
          role="option"
          @click.prevent="setActiveOption(list)"
        >
          <CurrencyIcon :name="list.text" />
          <span class="ml-3 font-normal block truncate text-sm">{{
            list.text
          }}</span>
    ...
    >
      <q-card>
        ...
        <VueScrollPicker v-model="localValue" :options="selectList">
          <template v-slot:default="{ option }">
            <div class="option-value flex items-center">
              <CurrencyIcon :name="option.text" />
...
<script>
import VError from 'components/extends/v-error/v-error.vue'
import { VueScrollPicker } from 'vue-scroll-picker'
import 'vue-scroll-picker/lib/style.css'
import { computed } from 'vue'
import CurrencyIcon from 'components/helper/icons/currency-icon.vue'
