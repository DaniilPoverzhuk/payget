<template>
  <tr class="table__tr tr-large">
    ...
    <td class="table__td">{{ $t(dealType) }}</td>
    <td class="table__td">
      <router-link class="table__link" :to="`/user/${dealCounterparty.name}`">{{
        dealCounterparty.name
      }}</router-link>
    </td>
    <td class="table__td">
      <Transaction
    ...
    <td class="table__td">{{ $t(dealStatus) }}</td>
        ...
        <tbody>
          <tr v-for="(item, idx) in headers" :key="item.label">
            ...
            <td v-else-if="idx === 4">
              <router-link
                class="table__link"
                :to="`/user/${dealCounterparty.name}`"
                >{{ dealCounterparty.name }}</router-link
              >
            </td>
            <td v-else-if="idx === 5">
              <Transaction
            ...
            <td v-else-if="idx === 6">{{ $t(dealStatus) }}</td>
...
import { DEAL_STATUS } from 'src/constants'
...
export default {
  ...
  data: () => ({
    dealTypeSale: 'purchase',
    dealTypePurchase: 'sale',
  }),
  props: {
    deal: {
      type: Object,
  ...
  computed: {
    ...mapUserState(['id', 'currencies', 'viewUserId']),
    userName() {
    ...
    dealType() {
      const userId = this.userName ? this.viewUserId : this.id

      return this.deal.seller.id === userId
        ? this.dealTypeSale
        : this.dealTypePurchase
    },
    diffDate() {
    ...
    dealCounterparty() {
      const { name, id } =
        this.dealType === this.dealTypeSale ? this.deal.buyer : this.deal.seller
